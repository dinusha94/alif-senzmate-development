...
{
    BOARD_BUTTON_STATE btn_state2;
    BOARD_BUTTON3_GetState(&btn_state2); // Read button state
    return (btn_state2 == BOARD_BUTTON_STATE_LOW); // Active low: true if pressed
}
...
{   
    ...
    while(1) {
        ...
        if (run_requested_btn_2()){
            ...
            hal_get_audio_data(audio_inf_kws, 32000); // recorded audio data in mono
            ...
            // info(" audio samples received : %d \n", hal_get_audio_samples_received());

            hal_audio_preprocessing(audio_inf_kws, 32000); 
